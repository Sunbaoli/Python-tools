...
# =======================================
'''
def resize_img(img, scale_w, scale_h):

    print('image size:  ', img.shape) 
    height, width = img.shape[:2]
    size = (int(width*scale_w), int(height*scale_h))
    shrink = cv2.resize(img, size, interpolation=cv2.INTER_CUBIC)
    #interp = ['cv2.INTER_CUBIC', INTER_NEAREST, INTER_LINEAR, INTER_AREA, INTER_LANCZOS4]
    print('shrink_size:  ', shrink.shape)
    return shrink
...
def slide_crop_image(img_path, box_w, box_h, stride, save_path):
    ...
    for filename in data:
        ...
        for y in range(0,img_h-box_h-1,stride):
            for x in range(0,img_w-box_w-1,stride):
                crop_img = img[(y):(y+box_h),(x):(x+box_w)]
                print('crop_img size:  ', crop_img.shape)
                if crop_img.shape[0]==box_h and crop_img.shape[1]==box_w:
                	cv2.imwrite(save_path+ str(i) + '_'+ str(count) +'.png', crop_img)
...
def count_images(img_path):
    ...
    for filename in data:
        count+=1
    print('This file has ' + str(count) + ' images')
